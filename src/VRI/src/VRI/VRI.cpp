...
TUnique<CFence> VRICreateFence(const bool inSignaled) {
    VkFenceCreateInfo fenceCreateInfo {
        .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
       .pNext = nullptr,
       .flags = inSignaled ? VK_FENCE_CREATE_SIGNALED_BIT : static_cast<VkFenceCreateFlags>(0)
   };
    TUnique<CFence> fence{fenceCreateInfo};
    //m_Allocator->m_Resources.push(fence);
    return std::move(fence);
}

TUnique<CSemaphore> VRICreateSemaphore() {
    VkSemaphoreCreateInfo semaphoreCreateInfo {
        .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
       .pNext = nullptr,
       .flags = 0
   };
    TUnique<CSemaphore> semaphore{semaphoreCreateInfo};
    //m_Allocator->m_Resources.push(semaphore);
    return std::move(semaphore);
}

TUnique<SSwapchainImage> VRICreateSwapchainImage() {
    return TUnique<SSwapchainImage>{};
}

TUnique<SSwapchainImage> VRICreateSwapchainImage(
    const VkImage inImage,
    const VkImageView inImageView,
    const uint32 inBindlessAddress
) {
