...
void CMeshPass::init(const TFrail<CRenderer> inRenderer) {
	...
	opaquePipeline = VRICreatePipeline(createInfo, attributes, CBindlessResources::getBasicPipelineLayout());
	...
	transparentPipeline = VRICreatePipeline(createInfo, attributes, CBindlessResources::getBasicPipelineLayout());
	...
	errorPipeline = VRICreatePipeline(createInfo, attributes, CBindlessResources::getBasicPipelineLayout());
	...
	wireframePipeline = VRICreatePipeline(createInfo, attributes, CBindlessResources::getBasicPipelineLayout());
...
void CMeshPass::render(const SRendererInfo& info, const TFrail<CVRICommands>& cmd) {
	...
	renderChild(info, this, cmd, stack, info.scene.get(), meshCount, drawCallCount, vertexCount);
